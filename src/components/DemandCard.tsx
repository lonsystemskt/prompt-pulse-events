...
export const DemandCard: React.FC<DemandCardProps> = ({
  ...
  const handleComplete = () => {
    console.log('DemandCard: Completando demanda', demand.id);
  ...
  const handleDelete = () => {
    console.log('DemandCard: Excluindo demanda', demand.id);
    onDeleteDemand(demand.id);
  };

  return (
    <div className="bg-slate-700/40 backdrop-blur-lg rounded-xl border border-slate-600/30 p-2 shadow-lg hover:shadow-xl transition-all duration-300 hover:bg-slate-700/50 w-[220px] h-[90px] min-w-[220px]">
